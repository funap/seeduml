...
var SequenceDiagram = class {
  ...
  returnMessage(text) {
    ...
    const step = this.addMessage(from, to, text, "dotted");
  ...
  parse(content) {
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (pendingNote) {
        ...
        if (isEndNote || isEndHnote || isEndRnote) {
          ...
          if (pendingNote.participants.length === 0) {
            if ((normPos === "right" || normPos === "left") && (lastMessageFrom || lastMessageTo)) {
              pendingNote.participants = [lastMessageFrom, lastMessageTo];
      ...
      const noteMatch = line.match(/^(h|r)?note\s+(left|right|over|across)(?:\s+(?:of\s+)?([^#:]+))?\s*(#\w+)?\s*(?::\s*(.*))?$/i);
      if (noteMatch) {
        ...
        if (text !== void 0) {
          ...
          if (participants.length === 0) {
            if ((normPos === "right" || normPos === "left") && (lastMessageFrom || lastMessageTo)) {
              participants = [lastMessageFrom, lastMessageTo];
              if (lastMessageStep !== -1) {
                associationStep = lastMessageStep;
              }
            }
      ...
      if (note.position === "across") {
        ...
        if (pIdxs.length > 0) {
          ...
          if (note.position === "left") {
            const currentLeft = stepOccupancy.get(key) ?? cx - 15;
            x = currentLeft - noteWidth;
            ...
            const baseRight = Math.max(15, activationOffset, selfMsgRightOffset);
            const currentRight = stepOccupancy.get(key) ?? cx + baseRight + 5;
            x = currentRight;
...
function renderAll(selector = "pre.nanouml") {
  ...
  blocks.forEach((block) => {
    ...
    container.className = "nanouml-diagram";
...
function initialize(config = {}) {
  const { startOnLoad = true, selector = "pre.nanouml" } = config;
